...
# scene_name = "stray_scan_02_sub5_all"  # Scan Name
# num_frames = 5508  # Number of frames to use (-1 for all)
# scene_name = "stray_scan_03_sub20_all"  # Scan Name
# num_frames = 893  # Number of frames to use (-1 for all)
# scene_name = "stray_scan_03_sub15_all"  # Scan Name
# num_frames = 1190  # Number of frames to use (-1 for all)
# scene_name = "stray_0403_sub30_all"  # Scan Name
# num_frames = 562  # Number of frames to use (-1 for all)
scene_name = "stray_0404_sub30_200"  # Scan Name
num_frames = 200  # Number of frames to use (-1 for all)
...
downscale_factor = 2.0  # Downscale for tracking (high res is slow)
...
config = dict(
    ...
    wandb=dict(
        ...
        project="SplaTAM",
        group="iPhone_StrayScan",
        name=f"stray_{scene_name}_{seed}",
        save_qual=False,
        eval_save_qual=True,
    ),
    data=dict(
        ...
        stride=1,
        num_frames=num_frames,
        # skip_mapping_frames="127-135,143-158",  # These frames are tracked but not mapped
    ),
    tracking=dict(
        use_gt_poses=False,
        forward_prop=True,
    ...
    mapping=dict(
        ...
        pruning_dict=dict(
            ...
            reset_opacities_every=50,
        ),
        use_gaussian_splatting_densification=False,
        densify_dict=dict(
            start_after=50,
            remove_big_after=300,
            stop_after=5000,
            densify_every=100,
            grad_thresh=0.0002,
            num_to_split_into=2,
